...
class Rule:
    def __init__(self, tokens, text, rule, trigger, controlled, controller, subtrigger, subcontrolled, subcontroller, base_rule_name):
        self.tokens = tokens
        self.text = text
        self.rule = rule
        ...
        self.subcontrolled = subcontrolled
...
for item in json_data:
    ...
    for new_subrule in new_subrules:
        ...
        proc_data_entry['subtrigger'] = subrules[0]['trigger']
        proc_data_entry['subcontrolled'] = subrules[0]['controlled']
        proc_data_entry['subcontroller'] = subrules[0]['controller']
...
for i in range(len(rule_list)):
    rule = rule_list[i]
    base_tokens = rule['sentence_tokens']
    ...
    subtrigger = rule['subtrigger']
    subcontrolled = rule['subcontrolled']
    subcontroller = rule['subcontroller']
    rule_name = rule['base_rule_name']
    new_tokens = add_special_tokens(tokens, controller_range, controlled_range, trigger_range)
    rule_data[rule_name].append(
        Rule(
            base_tokens, 
            ' '.join(new_tokens), 
            rule['rule'].strip(),
...
for key, value in val_rules.items():
    ...
    for i in tqdm(range(len(value))):
        ...
        rule = value[i].rule
        # relation arguments
        trigger = value[i].trigger
        controlled = value[i].controlled
        controller = value[i].controller
        # sub relation arguments
        subtrigger = value[i].subtrigger
        subcontrolled = value[i].subcontrolled
        subcontroller = value[i].subcontroller
        # base rule name
        base_rule_name = value[i].base_rule_name
        results.append(
            {
                "rule_name": key + '_' + str(i),
                "tokens": tokens, 
                "pred0": pred0, 
                "pred1": pred1, 
                "rule": rule,
